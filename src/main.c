...
typedef enum FitMode { FM_REAL_SIZE, FM_FIT, FM_FILL, FM_FIT_HORIZONTAL, FM_FIT_VERTICAL } FitMode;
...
void toggle_fullscreen() {
	...
	if (fullscreen) {
		...
		if (gotPlacement && gotMonitor) {
			...
			SetWindowPos(
				hwnd, HWND_TOP, mi.rcMonitor.left, mi.rcMonitor.top,
				windowWidth + 1, // if we don't do this we get a flicker when entering fullscreen...
				windowHeight + 1, SWP_NOOWNERZORDER | SWP_FRAMECHANGED);
		}
	} else {
		scaleA = 2;
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR lpCmdLine, INT nCmdShow) {
	...
	hwnd = CreateWindowExW( //
		...
		0,
#endif
		className, L"Tivi - Drop image files to view", WS_OVERLAPPEDWINDOW,
		// x,y
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE: {
		const PIXELFORMATDESCRIPTOR pfd = {
			...
			0,
			0,
			0, // redBits,redShift,greenBits,greenShift,blueBits,blueShift
			0,
			0, // alphaBits, alphaShift
			0, // accumBits
			0,
			0,
			0,
			...
			0,
			0,
			0 // layerMask,visibleMask,damageMask
		};
